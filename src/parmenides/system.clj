...
(defn dashboard-resources-components
  ...
  (assoc system
    ...
    (->
      (make new-static-resource-service config :uri-context "/" :resource-prefix "public/")
      (using [])
      (co-using []))))
...
(defn single-cljs-module-components
  ...
  (assoc system
    ...
    (->
      (make new-cljs-module config :name :cljs-core :mains [(quote cljs.core)] :dependencies #{})
      (using [])
      (co-using []))
    :single-cljs-module-cljs-app
    (->
      (make new-cljs-module config :name :default :mains [(quote parmenides.core)] :dependencies #{:cljs-core})
      (using [])
      (co-using []))
